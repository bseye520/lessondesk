...
给中小型培训机构（琴行、画室、舞蹈、书法、托管……）自建的**内部课时管理系统**。

## 它和大多数同类系统不一样在哪

**1. 这是给机构自己人用的，不是给家长学员用的。**
不开放注册，**没有家长端/学员端账号体系**。全校就三个角色 —— 系统管理员、校区管理员、教师，全部由机构内部建号。系统里装的是机构自己的账：谁带哪个班、上了几节课、谁还剩多少课时、收了多少钱、花了多少钱。

**2. 家长永远不用注册，也不用装任何东西。**
需要收报名时，管理员建一个「报名批次」，系统给一条一次性链接（`/r/<token>`），发到家长群里就行。家长点开、填孩子姓名和联系方式、选个班，完事 —— **不注册、不登录、不记密码、不用验证码**。收完这一批把链接一关，外面再也进不来。事后家长想改，凭报名时拿到的 6 位修改码自助修改。

**3. 多校区一起管。**
一套系统管多个校区：校区之间的学员、班级、教师、财务**互相隔离**，谁也看不到别人那一摊；总管理员一个后台看全局，随时切换。不用为每个校区建一套系统、对一遍数据。

**4. 只干一件事：谁还有多少课时，上过几次课，都扣了没。**
没塞课程商城、没做在线支付、没搞营销裂变、没接短信微信。那些你已经有别的工具了。这里就是一本**算得清、查得到、赖不掉的课时账**，加上一张收报名用的表。
...
> 先说清楚：下面有几条是**故意的设计取舍**，不是没做完。

- **没有家长端**（故意的）：家长只在「报名」那一刻出现，用一次性链接填个表就走。没有家长登录、没有家长查课时、没有家校互动。机构内部的事就在机构内部闭环
- **没有多租户**（故意的）：一套部署 = 一个机构（多个校区在机构内部管）
